...
- **Fixes, in order of effort:**
  ...
  2. **Cache revocation checks in memory** — since revocations are rare (only on logout/admin action), cache the revoked-jti set (or a "not revoked" positive cache with short TTL) in the process, invalidated on write. *Feasibility: moderate — needs care around multi-worker gunicorn processes not sharing memory (would need each worker to independently poll/refresh, which is fine since the set is small). Impact: removes 1 DB round trip from every authenticated request, not just gated ones — this is the higher-leverage of the two since it's checked on every request app-wide.*
     - **Redis as the proper fix for the multi-worker problem** — instead of each gunicorn worker maintaining its own in-process revocation cache (which means a token revoked in worker A isn't immediately known to workers B/C/D), a shared Redis instance would give all workers a single source of truth. A revocation write goes to Redis once; every worker reads from it. Redis also naturally supports TTL-based expiry (matching the token's own expiry) so the set never grows unbounded. Render offers a free Redis instance (Redis free tier: 25MB, which is more than enough for a small revoked-token set). *Feasibility: moderate — adds a new dependency (`redis-py`), needs a Redis URL env var wired up, and changes `token_in_blocklist_loader` and the logout/revocation write path. Not a large change but it touches auth, so warrants care. Impact: solves the multi-worker consistency problem cleanly, and removes the DB round trip on every request.*
