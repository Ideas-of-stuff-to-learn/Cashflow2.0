...
def run_llm_tier(pending_transactions: list, user_id: str, conn, batch_size: int = 200, gemini_timeout_ms: int = None) -> dict:
    ...
    if total_transactions > 0:
                timings['exact_percentage'] = round(
                    timings['exact_transactions'] / total_transactions * 100,
                    2
                )

                timings['merchant_percentage'] = round(
                    timings['merchant_transactions'] / total_transactions * 100,
                    2
                )

                timings['similarity_percentage'] = round(
                    timings['similarity_transactions'] / total_transactions * 100,
                    2
                )

                timings['gemini_percentage'] = round(
                    timings['gemini_transactions'] / total_transactions * 100,
                    2
                )
